...
-- ============================================================
-- Pink Bullet Tracer - Always ON
-- ============================================================
local PinkTracerEnabled = true
local PinkTracerColor = Color3.fromRGB(255, 105, 180)
local PinkTracerLifetime = 3
local PinkTracerFadeTime = 0.5
local PinkTracerSize = 1
local PinkTracers = {}
local PinkTracerHooked = false

local function PinkTracerMuzzle()
...
local function PinkMakeTracer(startPos, endPos)
    if not PinkTracerEnabled or not startPos or not endPos then return end

    local line = Drawing.new("Line")
    line.Thickness = 2 * PinkTracerSize
    line.Color = PinkTracerColor
    line.Transparency = 1
    line.Visible = false

    local outline = Drawing.new("Line")
    outline.Thickness = 4 * PinkTracerSize
    outline.Color = Color3.new(0, 0, 0)
    outline.Transparency = 1
    outline.Visible = false

    PinkTracers[#PinkTracers + 1] = {
        Line = line,
        Outline = outline,
        StartPos = startPos,
        EndPos = endPos,
...
local function PinkUpdateTracers()
    local camera = workspace.CurrentCamera
    if not camera then return end

    local now = tick()

    for i = #PinkTracers, 1, -1 do
        local tr = PinkTracers[i]
        local age = now - tr.Created

        if age >= PinkTracerLifetime then
            pcall(function() tr.Line:Remove() end)
            pcall(function() tr.Outline:Remove() end)
            table.remove(PinkTracers, i)
        else
            local fade = 1
            if PinkTracerFadeTime > 0 and age >= PinkTracerLifetime - PinkTracerFadeTime then
                fade = 1 - math.clamp(
                    (age - (PinkTracerLifetime - PinkTracerFadeTime)) / PinkTracerFadeTime,
                    0, 1
                )
            end

            local s, sVis = camera:WorldToViewportPoint(tr.StartPos)
            local e, eVis = camera:WorldToViewportPoint(tr.EndPos)

            if sVis and eVis and s.Z > 0 and e.Z > 0 then
...
RunService.RenderStepped:Connect(PinkUpdateTracers)

local function HookPinkTracer()
    if PinkTracerHooked then return true end
    ...
    local ok = pcall(function()
        ...
        if type(TracerEffect.Play) ~= "function" then return end

        local oldPlay = TracerEffect.Play

        TracerEffect.Play = function(self, tracerData, config, extraData)
            if PinkTracerEnabled and tracerData and tracerData.IsLocal then
                local muzzle = PinkTracerMuzzle()
                if muzzle and tracerData.RaycastResults then
                    for _, result in ipairs(tracerData.RaycastResults) do
                        if result and result.Position then
                            PinkMakeTracer(muzzle, result.Position)
                        end
                    end
                end
            end
...
task.defer(function()
    for _ = 1, 30 do
        if HookPinkTracer() then break end
        task.wait(0.5)
    end
end)
