...
-- ============================================================
-- Bullet Tracer Module - Mod Gun
-- Toggle + Color Picker
-- ============================================================
local BulletTracerModule = GunBox:AddDependencyBox()
BulletTracerModule:SetTitle("Bullet Tracer")
...
BulletTracerModule:AddToggle("BulletTracer", {
    Text = "Enable",
    Default = true,
...
BulletTracerModule:AddLabel("Tracer Color"):AddColorPicker("BulletTracerColor", {
...
local function BulletMakeTracer(startPos, endPos)
    if not BulletTracerEnabled or not startPos or not endPos then return end
...
local function BulletUpdateTracers()
    ...
    for i = #BulletTracers, 1, -1 do
        ...
        if age >= BulletTracerLifetime then
            pcall(function() tr.Line:Remove() end)
            pcall(function() tr.Outline:Remove() end)
            table.remove(BulletTracers, i)
        else
            local fade = 1
            if BulletTracerFadeTime > 0 and age >= BulletTracerLifetime - BulletTracerFadeTime then
                fade = 1 - math.clamp(
                    (age - (BulletTracerLifetime - BulletTracerFadeTime))
                    / BulletTracerFadeTime, 0, 1
                )
            end
            ...
            if sVis and eVis and s.Z > 0 and e.Z > 0 then
                ...
                tr.Line.Color = BulletTracerColor
...
RunService.RenderStepped:Connect(BulletUpdateTracers)

local function HookBulletTracer()
    if BulletTracerHooked then return true end
    ...
    local ok = pcall(function()
        ...
        TracerEffect.Play = function(self, tracerData, config, extraData)
            if BulletTracerEnabled and tracerData and tracerData.IsLocal then
...
task.defer(function()
    for _ = 1, 30 do
        if HookBulletTracer() then break end
        task.wait(0.5)
    end
end)

BulletTracerModule:SetupDependencies({})
